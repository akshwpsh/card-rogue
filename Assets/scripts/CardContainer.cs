...
public class CardContainer : MonoBehaviour {
    ...
    public void DestroyCard(CardWrapper card) {
        ...
        StartCoroutine(effectcard(card));
    }
    
    public IEnumerator effectcard(CardWrapper cardWrapper)
    {
        
        Card card = cardWrapper.gameObject.GetComponent<Card>();
        CardEffect[] effects = card.item.effects; 
        Destroy(card.gameObject);
        foreach (var effect in effects)
        {
            yield return StartCoroutine(effect.ApplyEffect());
        }
